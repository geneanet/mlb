...
`
	...
	// Now add sub2
	sub2 := &testutil.DummySubscriber{Wg: sync.WaitGroup{}}
	sub2.Wg.Add(1)
...
// waitSub is a helper function that waits for a testutil.DummySubscriber's WaitGroup with a timeout.
func waitSub(t *testing.T, sub *testutil.DummySubscriber, name string) {
	t.Helper()
...
`
	...
	runTest := func(t *testing.T, src string, setup func(dp *testutil.DummyProvider), verify func(list []*backend.Backend)) {
		block := testutil.ParseHCL(t, src)
		ctx := &hcl.EvalContext{}
